...
**Property Measured:** How predictable a piece of text is, i.e. the exponential of the average negative log-likelihood of the text. Lower perplexity means the text more predictable and AI-like, higher perplexity means more surprising, complex, and human-like.

**Output format:** Raw perplexity value (typically 20-300 for natural text), normalized to 0.0 to 1.0 scale using min-max normalization. 0.0 indicates "very high perplexity (human-like)" and 1.0 indicates "very low perplexity (AI-like)."
...
**How signals combine:**  
`final_confidence = (signal1 * 0.4) + (signal2 * 0.35) + (signal3 * 0.25)`  
LLM judgment is most reliable (0.4), stylometry is established (0.35), perplexity is calibration-dependent (0.25).
...
- How long did the writing process take? [dropdown number][dropdown: hours,days, weeks, months]
...
[requires checkbox]
- No.
- Yes, I used the following tools: [Text area, 50-500 words]
...
**After submission:**
* Update content status in database: status = "under_review"
...
- Process Duration: <number> <hours|days|weeks|months>
...
Decision: [Approve Appeal | Deny Appeal | Inconclusive - Need More Info]

If Approve: Why is the appeal valid?
[Text area]

If Deny: Why does the original decision stand?
[Text area]

Recommended Changes to System:
[Text area for bias/pattern observations]

[Submit Decision]

---

SYSTEM OUTCOME OPTIONS

If Reviewer Selects "Approve Appeal":
- Content status changes to: human_written
- Creator receives notification: "Appeal Approved"
- Audit log updated with reviewer decision
- Label on content changes to: Human-Written (with note: "This classification was reviewed and reversed")
- Case status: resolved_approved

If Reviewer Selects "Deny Appeal":
- Content status remains: AI-generated
- Creator receives notification: "Appeal Denied"
- Audit log updated with reviewer decision and reasoning
- Label on content remains unchanged
- Case status: resolved_denied

If Reviewer Selects "Inconclusive":
- Content status: pending_additional_review
- Reviewer can request additional information from creator
- System sends automated message to creator with specific questions
- Case remains open, clock resets
```
